...
export default function EngineerMasterPage() {
  ...
  const { engineers, addEngineer, updateEngineer, deleteEngineer } = useMasterData()
      ...
      <div className="flex-1 flex flex-col lg:ml-[60px] mt-[60px]">
        ...
        <main className="flex-1 overflow-auto p-4 md:p-8">
          <Card className="shadow-lg">
            <CardHeader>
              <CardTitle className="text-xl md:text-2xl font-bold">Engineer Master</CardTitle>
            </CardHeader>
                  ...
                  <TableBody>
                    {engineers.map((e) => (
                      <TableRow key={e.id}>
                        ...
                        <TableCell className="max-w-sm truncate">{e.address}</TableCell>
                        <TableCell className="text-right">
                          <Button
                            variant="outline"
                            className="mr-2 bg-transparent"
                            onClick={() => updateEngineer(e.id, { name: e.name })}
                          >
                            Edit
                          </Button>
                          <Button variant="destructive" onClick={() => deleteEngineer(e.id)}>
                            Delete
                          </Button>
                        </TableCell>
                      </TableRow>
