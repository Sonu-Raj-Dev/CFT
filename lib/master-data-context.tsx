...
export function MasterDataProvider({ children }: { children: ReactNode }) {
  ...
  const [rolePermissions, setRolePermissionsState] = useState<RPMap>(
    {} as RPMap
  );
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      try {
        ...
        console.log(
          "[v0] MasterDataProvider load error:",
          (e as Error).message
        );
        setCustomers([]);
        setEngineers([]);
  ...
  const setUserRoles = (userId: string, roles: Role[]) => {
    ...
    setUserRolesState((prev) => ({
      ...prev,
      [userId]: roles as unknown as string[],
    }));
  };
  const setRolePermissions = (role: Role, routes: any[]) => {
    ...
    setRolePermissionsState((prev) => ({
      ...prev,
      [role as unknown as string]: routes as any,
    }));
  };
